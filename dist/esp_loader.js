...
export class ESPLoader extends EventTarget {
    ...
    async checkCommand(opcode, buffer, checksum = 0, timeout = DEFAULT_TIMEOUT) {
        ...
        else if ([CHIP_FAMILY_ESP32, CHIP_FAMILY_ESP32S2, CHIP_FAMILY_ESP32S3].includes(this.chipFamily)) {
            statusLen = 4;
        }
        else {
    ...
    async flashBegin(size = 0, offset = 0, encrypted = false) {
        ...
        let flashWriteSize = this.getFlashWriteSize();
        if (!this.IS_STUB &&
            [CHIP_FAMILY_ESP32, CHIP_FAMILY_ESP32S2, CHIP_FAMILY_ESP32S3].includes(this.chipFamily)) {
